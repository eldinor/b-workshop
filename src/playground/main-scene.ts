...
import { RadialCloner } from "@/clonerSystem/radialCloner";
...
*/
      ...
      if (keyName === "n" || keyName === "N") {
        console.log("NNNN");
        //    (this.scene.activeCamera! as ArcRotateCamera).upVector = Vector3.Down();
    ...
    `;
        ...
        postProcess.onApply = function (effect) {
          ...
          effect.setFloat("vignetteOpacity", 0.5);
          effect.setFloat("brightness", 2.0);
          effect.setFloat("vignetteRoundness", 10.0);
        };
      }
      if (keyName === "u" || keyName === "U") {
        console.log("UUUU");

        const singleMeshes: any = [];

        singleMeshesList.map((m) => {
          return singleMeshes.push(this.scene.getMeshByName(m.name));
        });
        console.log(singleMeshes);

        const cloned = (this.scene.getMeshByName("cooler") as Mesh).clone(
          "cloned"
        );
        cloned.scaling.scaleInPlace(0.8);
        cloned.bakeCurrentTransformIntoVertices();

        const rc = new RadialCloner([cloned], this.scene, {
          count: 20,
          radius: 4,
        ...
        rc.root!.position = new Vector3(-5, 1, 0);
      }
      //
    }); // end event
    //
